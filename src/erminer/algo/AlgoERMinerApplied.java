...
import erminer.ERMinerNodeModel;
import erminer.algo.Sequence;
import erminer.algo.SequenceDatabase;
import ca.pfv.spmf.tools.MemoryLogger;

/**
 * This is based on the original implementation of the ERMiner algorithm for mining sequential rules
 ...
 * <br/><br/>
 * Changes were made by Manuel Wildner to use this algorithm inside a KNIME Node.
 *
 * @see Occurence
 * @see Sequence
 ...
 */
public class AlgoERMinerApplied {
	//*** for statistics ***/
	...
	// object to write the output file (not needed for KNIME node)
//	BufferedWriter writer = null; 
	 ...
	 */
	public void runAlgorithm(double minSupport, double minConfidence, SequenceDatabase database) throws IOException {
		...
		runAlgorithm(database, minsuppRelative, minConfidence);
	}
	 ...
	 * @exception IOException if error reading/writing files
	 */
	public void runAlgorithm(SequenceDatabase database, int relativeMinsup, double minConfidence) throws IOException {
...
//		countPruning = 0;
		...
		// if the database try to write.
		if(database == null){
			try {
				database.addSequence(new String[]{});
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		// but we don't use an output file for a KNIME node
//		writer = new BufferedWriter(new FileWriter(output)); 
		...
		// close the file (not needed for KNIME node)
//		writer.close();
	...
	private void saveRule(Set<Integer> tidsIJ, double confIJ, int[] itemsetI, int[] itemsetJ) {
		...
		ERMinerNodeModel.addRowToOutput((ruleCount-1), buffer.toString(), tidsIJ.size(), confIJ);
